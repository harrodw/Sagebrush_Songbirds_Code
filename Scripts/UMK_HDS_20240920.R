...
sage_est_plot <- ggplot(data = abund_est_sage, aes(x = Sage.Cover, y = exp(Predicted))) +
  ...
  labs(x = NULL, y = paste0(species_to_model, "/ha"),
       title = "% sage Cover") +
  theme_minimal()
...
raster::plot(ras_sage)

# Write a funras_sage# Write a function to log-transform rasters 
...
ras_agg_sage <- raster::aggregate(ras_sage, fact = 8, fun = mean)
ras_agg_pern <- raster::aggregate(ras_pern, fact = 8, fun = mean)
ras_agg_bg <- raster::aggregate(ras_bg , fact = 8, fun = mean)
ras_agg_elv <- raster::aggregate(ras_elv , fact = 8, fun = mean)

# Log-transform the rasters that need to be logged 
ras_ln_sage <- log_ras(ras_agg_sage)
ras_ln_bg <- log_ras(ras_agg_bg)
...
ras_scl_sage <- scale_ras(ras_ln_sage, mean_sage, sd_sage)
ras_scl_pern <- scale_ras(ras_agg_pern, mean_pern, sd_pern)
ras_scl_bg <- scale_ras(ras_ln_bg, mean_bg, sd_bg)
...
ras_sage_pred <- ras_pred(ras = ras_scl_sage,beta = beta_sage)
ras_pern_pred <- ras_pred(ras = ras_scl_pern, beta = beta_pern)
ras_bg_pred <- ras_pred(ras = ras_scl_bg, beta = beta_bg)
ras_elv_pred <- ras_pred(ras = ras_scl_elv, beta = beta_elv)
...
raster::hist(ras_pred, main = paste("predicted density of", species_to_model))
...
# Plot predicted bird abundance
umk_mod_abd <- tm_shape(ras_pred) +
...
# Export map
tmap_save(umk_mod_abd, paste0(plot_path, paste0(species_to_model, "_umk_mod_abd")))
