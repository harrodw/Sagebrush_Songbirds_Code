...
brsp <- sobs %>% 
  ...
  mutate(Years.Since.Fire = case_when(Year == "Y1" ~ 2022 - Fire.Year,
                                      Year == "Y2" ~ 2023 - Fire.Year,
                                      Year == "Y3" ~ 2024 - Fire.Year)) %>% 
  select(-Burn.Sevarity, -Perennial.Cover, - Fire.Distance,
...
# #Export data
# write.csv(brsp_obs, "Data\\Outputs\\brsp_obs_jags.csv")
# write.csv(brsp_count, "Data\\Outputs\\brsp_count_jags.csv")
