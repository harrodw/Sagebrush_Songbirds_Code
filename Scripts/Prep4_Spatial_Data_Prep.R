...
sage_type <- rast(paste0(ras_path, "sage_RnR.tif"))
...
Count.South <- terra::extract(x = south,
                            y = grid_buff,
                            fun = function(x) sum(x, na.rm = TRUE))

# Percent of area that burned
Count.Burned <- terra::extract(x = fires_ras,
                                   y = grid_buff,
                                   fun = function(x) sum(x, na.rm = TRUE))

# Total number of cells
Cell.Count <- terra::extract(x = ref_rast,
                                  y = grid_buff,
                                  fun = function(x) sum(x, na.rm = TRUE))

# sagebrush type
Sage.Type <- terra::extract(x = sage_type,
                              y = grid_buff,
                              fun = function(x) modal(x, na.rm = TRUE))
...
grid_covs$TRI <- TRI[,2]
grid_covs$Count.South <- Count.South[,2] 
grid_covs$Count.Burned <- Count.Burned[,2]
grid_covs$Cell.Count <- Cell.Count[,2]
grid_covs$Sage.Type <- Sage.Type[,2]
...
for(i in 1:(nyears-1)){ # Skip 2022. For some reason it doesn't work
  ...
  rdnbr.tmp <- terra::extract(x = rdnbr_ras, y = grid_buff, fun = function(x) mean(x, na.rm = TRUE))
  grid_covs_fire$rdnbr.tmp <- rdnbr.tmp[,2]
  ...
  grid_covs_fire <- grid_covs_fire %>% 
    mutate(rdnbr = case_when(!is.na(rdnbr.tmp) ~ rdnbr.tmp,
                                 TRUE ~ rdnbr),
           Fire.Count = case_when(! is.na(rdnbr.tmp) ~ Fire.Count + 1,
                                  TRUE ~ Fire.Count),
...
grid_covs_fire2 <- grid_covs_fire %>% 
  ...
  mutate(Fire.Year = case_when(Grid.ID == "UT-B02" ~ 2017, 
                               Grid.ID == "ID-C22" ~ 1800,
                               TRUE ~ Fire.Year),
         rdnbr = case_when(Grid.ID == "UT-B02" ~ mean(grid_covs_fire$rdnbr[which(grid_covs_fire$rdnbr > 0)]),
                                Grid.ID == "ID-C22" ~ 0,
                                TRUE ~ rdnbr),
         Fire.Count= case_when(Grid.ID == "UT-B02" ~ 1, 
                               Grid.ID == "ID-C22" ~ 0,
                               TRUE ~ Fire.Count)) %>% 
  # The Automated method for fire year has issues so I will manually calculate the fire years
  mutate(Fire.Year = case_when(
                               # Wagonbox, Dry Canyon
                               Grid.ID %in% c("UT-B15", "UT-B16", "UT-B25", "UT-C24", "UT-C25") ~ 1999,
                               # City of Rocks and Devine Canyon
                               Grid.ID %in% c("ID-B09", "ID-B28", "UT-B19") ~ 2000,
                               # Prospect
                               Grid.ID %in% c("UT-B17") ~ 2002,
                               # Burnt
                               Grid.ID %in% c("ID-B04", "ID-B22", "ID-B24") ~ 2006,
                               # Jim Sage, Black Pine 2, Winecup Complex
                               Grid.ID %in% c("ID-B07", "ID-B11", "ID-B21", "UT-B27", "UT-B30") ~ 2007,
                               # Emery
                               Grid.ID %in% c("ID-B12", "ID-B15") ~ 2010,
                               # Cave Canyon, Deer Hollow
                               Grid.ID %in% c("ID-B16", "ID-B19", "ID-B26", "ID-B23") ~ 2012,
                               # Playground
                               Grid.ID %in% c("UT-B01") ~ 2013,
                               # Rosebud
                               Grid.ID %in% c("UT-B02") ~ 2017,
                               # Goose Creek
                               Grid.ID %in% c("UT-B05", "UT-B06", "UT-B08", "UT-B22", "UT-B24") ~ 2018,
                               # Badger
                               Grid.ID %in% c("ID-B13") ~ 2020,
                               TRUE ~ 0))
...
# Turn count columns into proportions 
  mutate(Prop.South = 100 * Count.South / Cell.Count,
         Prob.Burned = 100 * Count.Burned / Cell.Count) %>% 
  # Remove the cells with only tiny south aspect slopes
  mutate(Prop.South = case_when(Prop.South < 1 ~ 0,
                                     Prop.South > 1 ~ Prop.South)) %>% 
  # Create a log-scale version of prop burned 
  mutate(ln.Prop.South = case_when(Prop.South == 0 ~ Prop.South,
                                        Prop.South > 0 ~ log(Prop.South))) %>% 
  # Remove the columns that are no longer needed
  select(-Count.South,
         -Cell.Count, 
         -Count.Burned) 

# Check to make sure there are no misisng fires
grid_covs_final %>% 
  filter(Grid.Type == "R") %>% 
  count(Fire.Year)
...
grid_covs_final %>% 
  filter(Grid.Type == "B") %>% 
ggplot(aes(x = Fire.Year, y = Shrub.Cover)) +
