...
counts_temp <- read.csv(paste0("Data/Outputs/VESP_Grid_Counts.csv")) %>%
  tibble() %>%
  select(-X)
#view the counts
glimpse(counts_temp)
...
observations_temp <- read.csv(paste0("Data/Outputs/VESP_Observations.csv"))
...
vesp_model_vect <- nimbleModel(vesp_model_code, 
                               data = vesp_dat,
                               constants = vesp_const,
                               dimensions = vesp_dims,
                               inits = vesp_inits)
...
# vesp_model_out_test <- nimbleMCMC(code = vesp_model_code,
#                                          constants = vesp_const,
#                                          data = vesp_dat,
#                                          dimensions = vesp_dims,
#                                          inits = vesp_inits,
#                                          monitors = vesp_params,
#                                          nchains = nc_test,
#                                          niter = ni_test, 
#                                          nburnin = nb_test, 
#                                          thin = nt_test, 
...
vesp_mcmcConf <- configureMCMC(vesp_model_vect, 
                               monitors = vesp_params)

# Block all availability (gamma) nodes together
vesp_mcmcConf$removeSamplers("gamma0", "gamma_date", "gamma_time", "gamma_date2", "gamma_time2")

vesp_mcmcConf$addSampler(target = c("gamma0", "gamma_date", "gamma_time", "gamma_date2", "gamma_time2"),
                         type = 'RW_block')

# Block all detection (alpha) nodes together
vesp_mcmcConf$removeSamplers(
                             # "alpha0",
...
vesp_mcmcConf$addSampler(target = c(
                                    # "alpha0", 
                                    "alpha0_obsv[1]", "alpha0_obsv[2]", "alpha0_obsv[3]", "alpha0_obsv[4]", 
                                    "alpha0_obsv[5]", "alpha0_obsv[6]", "alpha0_obsv[7]", "alpha0_obsv[8]",
                                    "alpha0_obsv[9]", "alpha0_obsv[10]", "alpha0_obsv[11]", "alpha0_obsv[12]", 
...
# View the blocks
vesp_mcmcConf$printSamplers() # print samplers being used 
vesp_mcmcConf$unsampledNodes  # look at unsampled nodes

# Build MCMC object: can customize thinning, monitors, etc.
vesp_modelMCMC <- buildMCMC(vesp_mcmcConf) # Update mcmc configuration based on graph
...
cModel <- compileNimble(vesp_model_vect) # compiled model
cMCMC <- compileNimble(vesp_modelMCMC, project = cModel, resetFunctions = T) # compiled mcmc
...
saveRDS(vesp_mcmc_out, file = "C://Users//willh//Box//Will_Harrod_MS_Project//Model_Files//VESP_fire_model.rds")
...
vesp_mcmc_out <- readRDS(file = paste0("C://Users//willh//Box//Will_Harrod_MS_Project//Model_Files//VESP_fire_model.rds"))
  
# Traceplots and density graphs 
MCMCtrace(object = vesp_mcmc_out$samples,
          params = vesp_params,
          pdf = TRUE,
          open_pdf = TRUE,
          ind = TRUE,
          ...
          filename = "VESP_fire_model_traceplot",
          type = 'both')

# View MCMC summary
MCMCsummary(object = vesp_mcmc_out$samples, 
            params = vesp_params,
            round = 2)
...
# View MCMC plot
MCMCplot(object = vesp_mcmc_out$samples,
         guide_lines = TRUE,
         labels = ,
         params = vesp_params_plot)
...
# Load the output back in
vesp_mcmc_out <- readRDS(file = paste0("C://Users//willh//Box//Will_Harrod_MS_Project//Model_Files//VESP_fire_model_linear.rds"))

# View MCMC summary
vesp_mcmc_out$summary$all.chains

# 4.1) Extract coefficient values ###############################################################

# Extract effect sizes
# Treatment intercepts
beta0_ref_low <- vesp_mcmc_out$summary$all.chains[18,]
beta0_ref_high <- vesp_mcmc_out$summary$all.chains[19,]
beta0_burn_low <- vesp_mcmc_out$summary$all.chains[20,]
beta0_burn_high <- vesp_mcmc_out$summary$all.chains[21,]
# Fire Year
beta_fyear_low <- vesp_mcmc_out$summary$all.chains[24,]
beta_fyear_high <- vesp_mcmc_out$summary$all.chains[25,]
# Burn Sevarity
beta_burnsev_low <- vesp_mcmc_out$summary$all.chains[22,]
beta_burnsev_high <- vesp_mcmc_out$summary$all.chains[23,]
...
treatment_pred_plot <- beta_dat %>% 
  ...
  scale_color_manual(values = c("beta0.ref.low" = "mediumseagreen",
                     ...
                     labels = c("Low Elevation Reference",
                     ...
                     name = "Treatment") +
  labs(x = "Treatment", 
       y = "Predicted Vesper Sparrow Abundance (birds/km^2)") +
  ylim(0, 100) +
  theme_classic() +
  theme(
