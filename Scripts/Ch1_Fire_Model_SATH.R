...
sath_mcmc_out <- readRDS(file = paste0("C://Users//willh//Box//Will_Harrod_MS_Project//Model_Files//", 
                                       "SATH", "_fire_model.rds"))
  
# Traceplots and density graphs 
MCMCtrace(object = sath_mcmc_out$samples,
          params = sobs_params,
          pdf = TRUE,
          open_pdf = TRUE,
          ind = TRUE,
...
# View MCMC summary
MCMCsummary(object = sath_mcmc_out$samples, 
            params = sobs_params,
            round = 2)

# View MCMC plot
MCMCplot(object = sath_mcmc_out$samples,
         guide_lines = TRUE,
         params = sobs_params)
...
# View MCMC summary
sath_mcmc_out$summary$all.chains

# 4.1) Extract coefficient values ###############################################################

# Extract Beta0 values
beta0_year1 <- sath_mcmc_out$summary$all.chains[19,]
beta0_year2 <- sath_mcmc_out$summary$all.chains[20,]
beta0_year3 <- sath_mcmc_out$summary$all.chains[21,]
# View intercepts
print(bind_rows(beta0_year1, beta0_year2, beta0_year3))

# Extract effect sizes
beta_burn <- sath_mcmc_out$summary$all.chains[22,]
beta_burnsev <- sath_mcmc_out$summary$all.chains[23,]
beta_elv <- sath_mcmc_out$summary$all.chains[24,]
beta_fyear <- sath_mcmc_out$summary$all.chains[25,]
